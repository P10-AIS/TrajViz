    ...
    for filename in os.listdir(directory):
        ...
        try:
            with np.load(path, allow_pickle=True) as data:
                ...
                clean_points = [
                    [None if (isinstance(val, float) and np.isnan(val))
                     else val for val in point]
                    for point in points
                ]
        ...
        for filename in os.listdir(data_dir):
            if filename.startswith("combined") and filename.endswith(".npz"):
                ...
                with np.load(path, allow_pickle=True) as data:
                    ...
                    trajectory_idxes = pickle.loads(
                        data['trajectory_idxes'].item())

                    # Transform to [trajectories, seq_len, 3] with None padding
                    formatted_data = parse_to_trajectories(
                        flat_trajectories, trajectory_idxes)
